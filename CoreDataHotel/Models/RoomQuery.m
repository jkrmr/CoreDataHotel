...
+ (NSFetchRequest*) availableOnDaysFrom:(NSDate*)startDate to:(NSDate*)endDate {
  NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:@"Room"];

  NSString *query = @"SUBQUERY(reservations, $r, $r.startDate < %@ AND $r.endDate > %@).@count == 0";
  NSPredicate *noConflictingBooking = [NSPredicate predicateWithFormat: query, endDate, startDate];
